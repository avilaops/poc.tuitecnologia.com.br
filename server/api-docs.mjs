...
export const openApiSpec = {
  ...
  paths: {
    ...
    '/api/sign/certificate': {
      get: { summary: 'Certificado ativo do serviço de assinatura (A1/A3)', responses: { 200: { description: 'Metadados do certificado e modo de operação' } } },
      delete: { summary: 'Remove o certificado enviado pela tela e volta ao do ambiente (perfil Administração)', responses: { 200: { description: 'Certificado revertido' }, 403: { description: 'Perfil sem permissão' } } },
    },
    '/api/sign/certificate/upload': { post: { summary: 'Instala certificado A1 (.pfx/.p12) enviado pela tela, validando arquivo e senha (perfil Administração)', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['p12Base64', 'password'], properties: { p12Base64: { type: 'string', description: 'Arquivo PKCS#12 em base64 (até 128 KB)' }, password: { type: 'string', format: 'password' } } } } } }, responses: { 201: { description: 'Certificado instalado e ativo' }, 400: { description: 'Arquivo ou senha inválidos, ou certificado expirado' }, 403: { description: 'Perfil sem permissão' } } } },
